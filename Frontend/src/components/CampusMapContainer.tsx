...
interface CampusMapContainerProps {
  ...
  onBuildingSelect: Building | null;
  isDarkMode?: boolean;
}

interface MapHandlerProps {
  bounds: typeof CAMPUS_INFO.bounds | undefined;
  mapRef: React.MutableRefObject<L.Map | null>;
  onMapClick: (latlng: L.LatLng) => void;
}

const calculatePathDistance = (pathCoords: number[][]): number | null => {
...
function MapHandler({ bounds, mapRef, onMapClick }: MapHandlerProps) {
  const map = useMap();
  mapRef.current = map;
  ...
  useEffect(() => {
    ...
    const campusCenter: LatLngExpression = [(bounds.north + bounds.south) / 2, (bounds.east + bounds.west) / 2];
...
export default function CampusMapContainer({ searchQuery, selectedCategory, onBuildingSelect, isDarkMode }: CampusMapContainerProps) {
  ...
  useEffect(() => {
    ...
    const watchId = navigator.geolocation.watchPosition(
      (pos) => { setUserLoc({ lat: pos.coords.latitude, lng: pos.coords.longitude }); setLocationStatus('Live GPS Active ✅'); },
  ...
  const getNearestNode = (lat: number, lng: number): string | null => {
  ...
  const handleBuildingClick = (building: Building) => {
    ...
    let shortestPath: any[] | null = null;

    const entranceList: string[] = Array.isArray(building.nearestNode)
      ? building.nearestNode
      : typeof building.nearestNode === 'string'
        ? building.nearestNode.split(',').map((s) => s.trim())
        : building.nearestNode ? [building.nearestNode as string] : [];

    entranceList.forEach((nodeId) => {
      const tempPath = pathFinder.find(startNode!, nodeId);
      if (tempPath && tempPath.length > 0) {
        if (!shortestPath || tempPath.length < shortestPath.length) shortestPath = tempPath;
      }
    });

    if (startNode && shortestPath) {
      const pathCoords = (shortestPath as any[]).map((n: any) => [n.data.lat, n.data.lng]).reverse();
      const fullPath = [[userLoc.lat, userLoc.lng], ...pathCoords];

      const line = L.polyline(fullPath as LatLngExpression[], {
      ...
      mapRef.current?.flyTo([Number(building.lat), Number(building.lng)], 18);
      setSelectedBuilding({ ...building, walking: undefined, loading: false });
    }
  };

  const filteredBuildings = useMemo(() => {
    let list = locations;
    if (selectedCategory && selectedCategory !== 'All') list = list.filter((b) => b.category === selectedCategory);
  ...
  const handleRecenter = () => { if (mapRef.current) mapRef.current.flyTo([userLoc.lat, userLoc.lng], 18); };

  return (
    <div className="w-full h-full relative flex flex-col lg:flex-row">
      <div className="flex-grow h-full w-full relative">
        <LeafletMap center={[userLoc.lat, userLoc.lng]} zoom={18} minZoom={16} maxZoom={21} zoomControl={false} style={{ height: '100%', width: '100%' }}>
          ...
          {filteredBuildings?.map((b) => (
            <Marker key={b.id} position={[Number(b.lat), Number(b.lng)]} icon={createIcon(getMarkerColor(b.category))} eventHandlers={{ click: () => handleBuildingClick(b) }}>
              <Popup>{b.name}</Popup>
            </Marker>
          ))}
        </LeafletMap>
        ...
        <div className="absolute bottom-6 left-6 z-[500] flex flex-col items-start gap-2">
          ...
          <button onClick={handleRecenter} className={`${isDarkMode ? 'bg-slate-800 border-slate-700' : 'bg-white border-gray-200 hover:bg-gray-50'} p-3 rounded-full shadow-xl border active:scale-95 transition-all flex items-center`}>
      ...
      {selectedBuilding && (
        <aside className={`fixed bottom-0 left-0 right-0 z-[1000] ${isDarkMode ? 'bg-slate-900 border-slate-700 text-white' : 'bg-white border-l text-black'} rounded-t-3xl shadow-[0_-10px_25px_rgba(0,0,0,0.1)] lg:relative lg:w-96 lg:h-full lg:rounded-none lg:border-l`}>
          <div className={`flex items-center justify-between p-4 border-b ${isDarkMode ? 'border-slate-700' : 'border-gray-200'}`}>
            ...
            <button onClick={() => { setSelectedBuilding(null); setSelectedBuildingId?.(null); }} className="p-1 hover:bg-gray-100/10 rounded-full transition-colors">
          ...
          <div className="max-h-[50vh] lg:max-h-[calc(100%_-_120px)] overflow-y-auto">
            <BuildingDetailsPanel
              building={{
                ...selectedBuilding,
                floorInfo: selectedBuilding.floorInfo || (selectedBuilding.floorinfo as any),
              }}
              onViewDetail={(b) => {
