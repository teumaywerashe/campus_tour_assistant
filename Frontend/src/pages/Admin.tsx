...
export default function Admin() {
  ...
  const handleNodeToggle = (nodeId: string) => {
    ...
    const newNodes = currentNodes.includes(nodeId) ? currentNodes.filter((id) => id !== nodeId) : [...currentNodes, nodeId];
  ...
  const handleDelete = async (id: number | string) => {
    ...
    try {
      ...
      if (response.data.success) toast.success('Building deleted successfully');
      fetchBuildings();
    } catch (err) {
      console.error(err);
  ...
  const inputBg = darkMode ? '#1e293b' : 'white';
          ...
          <div className="space-y-4 max-h-[75vh] overflow-y-auto pr-2">
            ...
            <div className={`w-full h-64 border rounded-lg overflow-hidden relative ${darkMode ? 'border-slate-700' : 'border-gray-200'}`}>
              <MapContainer center={[9.0409, 38.7621]} zoom={17} style={{ height: '100%', width: '100%' }}>
                ...
                {Object.entries(nodes).map(([id, coords]) => (
                  <CircleMarker key={id} center={[coords.lat, coords.lng]} radius={6}
                    pathOptions={{ color: formData.nearestNode.includes(id) ? '#4f46e5' : '#94a3b8', fillOpacity: 1, fillColor: formData.nearestNode.includes(id) ? '#818cf8' : '#cbd5e1' }}
              ...
              <div className="absolute bottom-2 left-2 z-[1000] pointer-events-none">
                <span className={`px-2 py-1 rounded text-[10px] font-bold shadow-sm border ${darkMode ? 'bg-slate-800/90 text-indigo-400 border-slate-700' : 'bg-white/90 text-indigo-600 border-gray-200'}`}>
            ...
            {/* Image Upload */}
            <div className={`flex items-center gap-4 p-3 border rounded-xl shadow-sm ${darkMode ? 'bg-slate-900 border-slate-700 text-white' : 'bg-white border-gray-200 text-black'}`}>
              <img src={imagePreview || editingBuilding?.images || DEFAULT_IMAGE} alt="Preview" className="w-16 h-16 object-cover shadow-md border dark:border-slate-800 rounded-lg" />
              <label className="flex-1 cursor-pointer flex items-center gap-3 text-sm font-bold py-2 px-3 hover:text-blue-500 group">
            ...
            <input className="admin-input" placeholder="Tags like {a,b,c}" value={formData.tags} onChange={(e) => setFormData({ ...formData, tags: e.target.value })} />
