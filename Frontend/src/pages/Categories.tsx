...
import { BookOpen, Car, TreePine, Dumbbell, GraduationCap, MapPin, ArrowUpRight, LayoutGrid } from 'lucide-react';
...
export default function Categories() {
  ...
  const filteredLocations = activeTab === 'All' ? locations : locations.filter((loc) => loc.category === activeTab);
  ...
  return (
    <div className={`min-h-screen w-full transition-colors duration-500 overflow-x-hidden ${darkMode ? 'bg-slate-950 text-white' : 'bg-white text-slate-900'}`}>
      <div className={`sticky top-0 z-50 w-full backdrop-blur-md border-b ${darkMode ? 'bg-slate-950/80 border-white/10' : 'bg-white/80 border-slate-200'}`}>
            ...
            {categoryButtons.map((cat) => {
              ...
              return (
                <button key={cat.id} onClick={() => setActiveTab(cat.id)}
                  className={`relative flex items-center gap-2 px-4 py-2 rounded-full font-bold transition-all flex-shrink-0 ${isActive ? 'opacity-100' : 'opacity-60'}`}>
                  {isActive && <motion.div layoutId="activeTabPill" className={`absolute inset-0 rounded-full ${darkMode ? 'bg-white/10' : 'bg-slate-100'}`} />}
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <header className="mb-10">
          <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} key={activeTab}>
            ...
            <p className="text-sm opacity-50 mt-1 font-medium">{filteredLocations.length} locations found</p>
          </motion.div>
        </header>
              ...
              <motion.div layout initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, scale: 0.9 }} key={location.id} className="w-full">
                <Link to={`/search?q=${location.name}`} className="group block">
                  ...
                  <div className="mt-4 px-1">
                    <h3 className="text-lg font-bold leading-tight group-hover:text-[#646cff] transition-colors">{location.name}</h3>
                    <div className="flex flex-wrap items-center gap-3 mt-2">
                      <div className="flex items-center gap-1 text-[10px] font-bold uppercase tracking-widest opacity-60">
                        ...
                        <span className="truncate max-w-[120px]">{(location as any).floorInfo?.depts?.[0] || 'Campus Area'}</span>
                      </div>
                      {activeTab === 'All' && (
                        <span className={`text-[8px] font-black uppercase tracking-widest px-2 py-0.5 rounded ${darkMode ? 'bg-white/5 text-slate-400' : 'bg-slate-100 text-slate-500'}`}>{location.category}</span>
                      )}
                    </div>
                  </div>
                </Link>
