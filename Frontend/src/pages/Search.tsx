...
interface SearchProps {
  ...
  onBuildingSelect: any;
  setOnBuildingSelect: (b: any) => void;
}

export default function Search({ searchQuery, setSearchQuery, selectedCategory, setSelectedCategory, onBuildingSelect, setOnBuildingSelect }: SearchProps) {
  ...
  useEffect(() => {
    const handleClickOutside = (event: MouseEvent) => {
      if (searchWrapperRef.current && !searchWrapperRef.current.contains(event.target as Node)) setIsOpen(false);
    };
  ...
  const filteredLocations = locations.filter((location) => {
    ...
    const matchesQuery = location.name.toLowerCase().includes(q) || (location.tags && location.tags.some((tag) => tag.toLowerCase().includes(q)));
  ...
  return (
    <div className={`relative h-[calc(100vh-64px)] w-full overflow-hidden ${darkMode ? 'bg-slate-950' : 'bg-white'}`}>
      <div ref={searchWrapperRef} className="absolute top-6 left-1/2 -translate-x-1/2 z-[1001] w-[90%] max-w-md">
        <div className={`${darkMode ? 'bg-slate-900 border-slate-700 shadow-black' : 'bg-white border-slate-200 shadow-2xl'} rounded-2xl border overflow-hidden transition-colors duration-300`}>
          <div className="relative flex items-center p-1">
            <div className="pl-4 flex items-center pointer-events-none"><SearchIcon className="h-5 w-5 text-slate-400" /></div>
            <input
              type="text"
              value={searchQuery}
            ...
            />
            {searchQuery && (
              <button onClick={() => setSearchQuery('')} className="pr-4 text-slate-400 hover:text-slate-600"><X className="h-5 w-5" /></button>
            )}
          </div>
          {isOpen && searchQuery.length > 0 && (
            <div className={`max-h-[50vh] overflow-y-auto border-t ${darkMode ? 'border-slate-700 bg-slate-900' : 'border-slate-100 bg-white'}`}>
              ...
              {filteredLocations.length === 0 && <div className="p-6 text-center text-slate-400 text-sm italic">No buildings found</div>}
            </div>
          )}
        </div>
      </div>
      <div className="absolute inset-0 z-0">
        <CampusMapContainer searchQuery={searchQuery} selectedCategory={selectedCategory} onBuildingSelect={onBuildingSelect} isDarkMode={darkMode} />
      </div>
    </div>
  );
}
