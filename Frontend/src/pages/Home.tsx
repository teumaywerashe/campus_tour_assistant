...
export default function Home() {
  ...
  return (
    <div className={`min-h-screen ${theme.bg} ${theme.text} transition-colors duration-300`}>
      <div className="relative h-[60vh] min-h-[450px] flex flex-col items-center justify-center text-center px-4 bg-cover bg-center" style={{ backgroundImage: "url('/gate.jpg')" }}>
        <div className="absolute inset-0 bg-black/40 z-0" />
        <div className="relative z-10 max-w-4xl w-full">
          <h1 className="text-4xl md:text-6xl font-black text-white mb-2 drop-shadow-md">WELCOME TO OUR</h1>
          <h2 className="text-3xl md:text-5xl font-black text-white mb-4 drop-shadow-md">CAMPUS TOUR ASSISTANT</h2>
          <p className="text-lg md:text-xl text-white/90 mb-8 font-medium">Discover, Navigate, Explore.</p>
      ...
      <div className="max-w-7xl mx-auto px-6 py-16">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          <Link to="/categories" className={`${theme.card} p-8 rounded-3xl shadow-xl hover:scale-105 transition-transform border flex flex-col items-center text-center`}>
            <div className="p-4 bg-blue-50 dark:bg-blue-900/20 rounded-2xl mb-4"><Search className="h-10 w-10 text-blue-500" /></div>
          ...
          <Link to="/search" className={`${theme.card} p-8 rounded-3xl shadow-xl hover:scale-105 transition-transform border flex flex-col items-center text-center`}>
            <div className="p-4 bg-cyan-50 dark:bg-cyan-900/20 rounded-2xl mb-4"><Globe className="h-10 w-10 text-cyan-500" /></div>
          ...
          <Link to="/about" className={`${theme.card} p-8 rounded-3xl shadow-xl hover:scale-105 transition-transform border flex flex-col items-center text-center`}>
            <div className="p-4 bg-emerald-50 dark:bg-emerald-900/20 rounded-2xl mb-4"><Building2 className="h-10 w-10 text-emerald-500" /></div>
      ...
      <div className="max-w-7xl mx-auto px-6 pb-20">
          ...
          {locations.slice(0, 5).map((loc) => (
            <Link key={loc.id} to={`/location/${loc.id}`} className="group">
              <div className="aspect-[4/3] rounded-2xl overflow-hidden mb-3 shadow-md">
                <img src={loc.images ? `${url}/uploads/buildings/${loc.images}` : '/placeholder.jpg'} alt={loc.name} className="w-full h-full object-cover group-hover:scale-110 transition-transform duration-500" />
              </div>
              <p className={`text-center font-semibold text-sm ${theme.textMuted} group-hover:text-blue-500 transition-colors`}>{loc.name}</p>
            </Link>
          ))}
        ...
        <div className="mt-6 flex justify-center">
          <Link to="/categories" className="px-4 py-2 bg-blue-600 text-white rounded-full text-sm font-semibold hover:opacity-90">See all categories</Link>
        </div>
      </div>
    </div>
  );
